...
- **Tier-2 distill catalog**: `/agent end` injects current `derived_profile.insights` (≤20) into the same Grok distill call. Same lived mechanism → exact stored label (`+0.08`); different friction or doubt → new label. No cosine merge. Distill defines `trigger` (cue that sets a pattern off) vs `habit` (what you then do) so new insights can fill the Triggers group.
