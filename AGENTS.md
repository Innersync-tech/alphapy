...
- **Check-in nudges (Phase 5A)**: Opt-in Discord DMs (`agent_prefs.agent_nudges_enabled`, default off). Hourly loop sends at most one fixed English invite embed per 24h (`agents/nudges.py` + Railway `agent_nudge_state`, Alembic `027`). No Grok / no journal text. Requires `/link` + mutual guild with `agents.enabled`. Does not consume session quota. GDPR: `agent_nudge_state` purged with Railway PII.
