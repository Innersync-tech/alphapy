...
- **Locale**: `utils/platform_locale.py` reads Core `bot-profile.locale` (`nl-BE` \| `en`, default `en`). Unlinked Discord → `en`. Agent replies use `locale_output_instruction`. Tier-2 distill uses `distill_locale_output_instruction` (new labels follow locale; stored catalog phrases stay). Not Discord guild language.
...
- **Enable gate**: `growth.enabled` (default true). When false, `/growthcheckin` and `/growthhistory` refuse with an ephemeral message; Growth modal submit and share buttons re-check. Admin `/growth set_channel` stays available. Dashboard Disable writes the same flag; the bot reloads settings via `ensure_fresh` / invalidate-cache (not only at startup).
...
- **Storage**: Each `/growthcheckin` inserts `goal` / `obstacle` / `feeling` / `grok_response` into Railway `growth_checkins` (canonical). Optional Supabase `reflections` sync remains for AI context when a profile is linked — **not** the Hermit progress source of truth.
- **Hermit/Core broker**: `GET /api/hermit/growth-checkins` (service `API_KEY`) exposes Railway rows for Core `GET /integrations/hermit/growthcheckins`.
...
- **Commands**: `/agent list`, `/agent start`, `/agent continue`, `/agent end`, `/agent status`, `/agent nudges enable|disable`
- **Admin config**: `/config agents show`, `/config agents toggle`
- **Check-in nudges (Phase 5A)**: Opt-in Discord DMs (`agent_prefs.agent_nudges_enabled`, default off). Hourly loop sends at most one fixed English invite embed per Europe/Brussels calendar day in the 20:00 local hour (`NUDGE_LOCAL_HOUR`, `agents/nudges.py` + Railway `agent_nudge_state`, Alembic `027`). No Grok / no journal text. Requires `/link` + mutual guild with `agents.enabled`. Does not consume session quota. GDPR: `agent_nudge_state` purged with Railway PII.
...
- **Tier-2 distill catalog**: `/agent end` injects current `derived_profile.insights` (≤20) into the same Grok distill call. Same lived mechanism → exact stored label (`+0.08`); different friction or doubt → new label. Catalog language is sticky (first write wins); locale only invents a phrase when the mechanism is new. No cosine merge. Distill defines `trigger` (cue that sets a pattern off) vs `habit` (what you then do) so new insights can fill the Triggers group.
- **Session write-back**: `agent_prefs.agent_writeback_enabled` (App `/dashboard/agent` Learning, default off). When on, `/agent end` may distill labels from the session transcript into Patterns even without shared reflections. Legacy `learn_from_shared` still requires active consent + journal context.
- **Pattern context**: `agents/pattern_loader.py` reads Tier-2 `derived_profile.insights` only (never graph progress / heuristic labels); prefs `learn_from_patterns` / fallback `learn_from_shared`. Distill `INSIGHT_TYPE_RULES` define trigger (cue) vs habit (reaction).
- **Output locale**: agent replies follow Innersync ID `preferences.locale` via `locale_output_instruction`. `/agent end` distill uses `distill_catalog_system_rules` (keep-apart last) so stored labels are not translated (see InnersyncIdentity)
...
- **Commands**: `/version`, `/gptstatus`, `/innersync`, `/release` (owner only; optional `product` Alphapy|App, optional `version`), `/health`, `/commands`, `/command_stats`, `/help`
