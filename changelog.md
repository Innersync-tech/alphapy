...
### Changed
- **Docs honesty (#416)** — `embedwatcher.failed_parse_log_channel_id` (Dashboard-only, falls back to system log); `guild_rules` image columns in English; `agent_sessions.status` is Core CHECK vs Alphapy write path (`active`/`completed` only).
...
### Fixed
- **Commands docs** — `pattern_loader` documents Tier-2 `derived_profile` (not `agent_graph_nodes`) after Pattern Pipeline Clean.
